module.exports = function (sequelize) {
  ...
  var modelCampaignRefCode = sequelize.import(__dirname + '/modelcampaign.js');

  model.hasMany(modelCampaignRefCode, {as: 'ReferrerCodes'});
  ...
  return {
    ...
    /**
     * updateCampaignReferrerCode( email, data, callback )
     * -
     * email: email address
     * data: JSON object containing user fields
     * callback: function( err, user )
     */
    updateEngagedWithCampaign: function ( email, campaignRefCode, callback ) {
      this.getUserByEmail( email, function( err, user ) {
        if ( err ) {
          return callback( err );
        }
        ...
        modelCampaignRefCode.findOrCreate(desiredRecord, desiredRecord).success(function(campaign, created) {
          if (created) {
            // this was new and has now been saved
            callback();
          } else {
            // save this again to update the autogenerated updatedAt field
            campaign.save().complete( callback );
          }
        });
      });
    }
